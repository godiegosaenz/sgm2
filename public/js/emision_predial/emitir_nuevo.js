			...
			$.each(data.resultado,function(i, item){
				...
				$('#tableRural').append(`<tr>
                                                ...
                                                <td style="width:10%;  text-align:left; vertical-align:middle">
                                                    <input class="form-check-input" type="checkbox" value=${item.CarVe_NumTitulo} name="checkPredio[]">
                                                </td>
...
function generarTitulos(){
    if ($('input[name="checkPredio[]"]:checked').length === 0) {
    ...
    $.ajaxSetup({
     ...
     var FrmData=$("#formEmision").serialize();
...
function buscarPredios(cedula, nombres, correos){
    ...
    // buscar-liquidacion-rurales-baja

    let ruta_busqueda='buscar-predios-rurales-emi/'+cedula+'/'
    ...
    $.get(ruta_busqueda, function(data){
        ...
        $.each(data.resultado,function(i, item){
            ...
            $('#tbodyRuralDetalle').append(`<tr>
                   <td style="width:5%; text-align:center; vertical-align:middle">
                        <input type="checkbox" name="checkPredio[]" class="predio_valor" id="predio_valor" value="${item.Pre_CodigoCatastral}"
                        data-valor_comercial="${item.valor_comercial}" data-base_imponible="${item.base_imponible}" data-sitio_barrio="${item.Ubi_Descripcion}"
                        data-nombre_predio="${item.Pre_NombrePredio}" >                
                    </td>
                    <td style="width:10%; text-align:center; vertical-align:middle">
                        ${item.Pre_CodigoCatastral}                      
                    </td>
                    <td style="width:20%; text-align:center; vertical-align:middle">
                        ${item.Ubi_Descripcion}                     
                    </td>
                    <td style="width:25%; text-align:center; vertical-align:middle">
                        ${item.Pre_NombrePredio}                      
                    </td>
                    <td style="width:30%; text-align:center; vertical-align:middle">
                        ${item.valor_terreno}                   
                    </td>
                    <td style="width:15%; text-align:center; vertical-align:middle">
                        ${item.valor_edif_predio}                     
                    </td>
                    <td style="width:15%; text-align:center; vertical-align:middle">
                        ${item.otras_inv}                     
                    </td>
...
globalThis.ValorComercial=[]
globalThis.BaseImponible=[]
globalThis.SitioBarrio=[]
globalThis.NombrePredio=[]
$(document).on('change', '.predio_valor', function () {
   
    let valor_comercial = $(this).data('valor_comercial');
    let base_imponible=$(this).data('base_imponible');
    let sitio_barrio=$(this).data('sitio_barrio');
    let nombre_predio=$(this).data('nombre_predio');
    if ($(this).is(':checked')) {

        // Agregar al array
        ValorComercial.push(valor_comercial);
        BaseImponible.push(base_imponible)
        SitioBarrio.push(sitio_barrio)
        NombrePredio.push(nombre_predio)

    } else {

        // Eliminar del array si se desmarca
        ValorComercial = ValorComercial.filter(
            item => item != valor_comercial
        );

        BaseImponible = BaseImponible.filter(
            item => item != base_imponible
        );

        SitioBarrio = SitioBarrio.filter(
            item => item != sitio_barrio
        );

        NombrePredio = NombrePredio.filter(
            item => item != nombre_predio
        );


    }

    console.log(ValorComercial);
});
function ejecuta(){
...
$(document).on('change', '#selectAll', function() {
    ...
    $('#tbodyRuralDetalle input[type="checkbox"]').each(function() {
     ...
     $('.predio_valor').prop('checked', $(this).is(':checked'));
    ...
    if ($(this).is(':checked')) {
        ...
        $('.predio_valor').each(function () {
            ...
            if (!ValorComercial.includes(valor_comercial)) {
                ValorComercial.push(valor_comercial);
            }

            if (!BaseImponible.includes(base_imponible)) {
                BaseImponible.push(base_imponible);
            }

            if (!SitioBarrio.includes(sitio_barrio)) {
                SitioBarrio.push(sitio_barrio);
            }

            if (!NombrePredio.includes(nombre_predio)) {
                NombrePredio.push(nombre_predio);
            }
...
function simularEmision(){
    ...
    // alert(urbano_rural) ver-titulo-rural-sin-pagar
    if ($('input[name="checkPredio[]"]:checked').length === 0) {
        alert("Debe seleccionar al menos un predio.");
        return false; // Previene la acción, si estás dentro de un submit o evento
    }
    ...
    ruta="simulacion-emision-rural-new"
    if(urbano_rural==1){
        ruta="tituloscoactivaurb/imprimir"
    }
    var FrmData=$("#formEmision").serialize();
    // FrmData += '&ValorComercial=' + JSON.stringify(ValorComercial);
    ValorComercial.forEach(function(item) {
        FrmData += '&ValorComercial[]=' + encodeURIComponent(item);
    });

    BaseImponible.forEach(function(item) {
        FrmData += '&BaseImponible[]=' + encodeURIComponent(item);
    });

    SitioBarrio.forEach(function(item) {
        FrmData += '&SitioBarrio[]=' + encodeURIComponent(item);
    });

    NombrePredio.forEach(function(item) {
        FrmData += '&NombrePredio[]=' + encodeURIComponent(item);
    });
