   ...
   <header>
    ...
    <div>
        ...
        <p style="font-size: 14px; text-align: justify; line-height: 1;">
            ...
            <b>SEGUNDO:</b>{!! ucfirst($txt_compareciente) !!}, a partir de la fecha de esta notificación pagara la suma de <b>(USD ${{$convenio->cuotas[1]->valor_cuota  }}) {{ numeroEnLetras2($convenio->cuotas[1]->valor_cuota ) }}, más intereses por {{$convenio->numero_cuotas}} meses contados a partir de la presente fecha</b>, estableciéndose la fecha de pago el {{ date('d') }} de cada mes y un saldo de <b>(USD. ${{$diferencia}}) {{ numeroEnLetras2($diferencia) }}</b> <br><br>
