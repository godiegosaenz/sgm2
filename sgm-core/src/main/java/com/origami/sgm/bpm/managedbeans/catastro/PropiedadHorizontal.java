 ...
 * dfcalderio
 */
@Named(value = "propiedadHorizontal")
@ViewScoped
public class PropiedadHorizontal implements Serializable {
    ...
    protected CatPredio predioMatriz, predioDepartamento;
    ...
    @PostConstruct
    public void init() {
        predios = new CatPredioLazy("A", Boolean.TRUE);
        ...
        predioDepartamento = new CatPredio();
    }
    ...
    public void grabarCuadroAlicuotas() {
        try {
            if (validarSumaAlicuotas()) {
                ...
                Faces.update("growl");
                Faces.redirectFaces("/faces/vistaprocesos/catastro/gestionarPH/propiedadHorizontal.xhtml");
            } else {
    ...
    public void ver(boolean edt) {
        if (predioMatriz != null && predioSeleccionado.getId() == null) {
            redirectPage(predioMatriz, edt);
        } else {
            if (predioSeleccionado != null) {
                redirectPage(predioSeleccionado, edt);
            }
        }
    }

    public void redirectPage(CatPredio cp, boolean edt) {
        ss.borrarDatos();
        ss.borrarParametros();
        ss.instanciarParametros();
        ss.agregarParametro("numPredio", cp.getNumPredio());
        ss.agregarParametro("idPredio", cp.getId());
    ...
    public void imprimirFicha(Boolean controlCroquis) {
        if (predioMatriz != null) {
            ...
            if (predioMatriz.getPredioRaiz() == null) {
                fotos = manager.findAll(Querys.getFotosIdPredio, new String[]{"predio"}, new Object[]{predioMatriz.getId()});
            } else {
                fotos = manager.findAll(Querys.getFotosIdPredio, new String[]{"predio"}, new Object[]{predioMatriz.getPredioRaiz().longValue()});
            }
            if (Utils.isNotEmpty(fotos)) {
                ...
                ss.agregarParametro("predio", predioMatriz.getId());
                ss.setNombreReporte("Ficha predial " + predioMatriz.getNumPredio());
                ss.setTieneDatasource(Boolean.TRUE);
                if (predioMatriz.getPredioRaiz() != null) {
                    CatPredio predioRaiz = (CatPredio) manager.find(Querys.getPrediosById, new String[]{"predioID"}, new Object[]{predioMatriz.getPredioRaiz()});
                    ss.agregarParametro("IMAGEN_PREDIO", SisVars.URLPLANOIMAGENPREDIO + predioRaiz.getNumPredio());
                } else {
                    ss.agregarParametro("IMAGEN_PREDIO", SisVars.URLPLANOIMAGENPREDIO + predioMatriz.getNumPredio());
                }
            }
            ss.agregarParametro("predio", predioMatriz.getId());
            ...
            if (!controlCroquis) {
                ...
                ed = manager.findAll(Querys.getCatPredioEdificacionByPredio, new String[]{"predio"}, new Object[]{predioMatriz.getId()});
                ...
                //ss.setReportePDF(reporte.generarPdf("/reportes/catastro/San Vicente/croquis.jasper", ss.getParametros()));
            }
    ...
    public void procesarCuadroAlicuotas() {
        ...
        if (cantidad != escritura.getCantAlicuotas()) {
    ...
    public void setUbicaciones() {
        ///VARIABLE DE PROPIETARIOS SE LA USA PARA SETEAR LAS UBICACIONES 
        String ubicacionesDep = "";
        String ubicacionesBo = "";
        List<CatPredio> catPredios = new ArrayList<>();
        for (CatPredio cp : this.prediosGenerados) {
            ubicacionesDep = "";
            ubicacionesBo = "";
            for (CatPredioAlicuotaComponente cpa : cp.getAlicuotaComponentes()) {
                if (cpa.getNombre().contains("Dep")) {
                    ubicacionesDep = cpa.getNombre() + " " + cpa.getDescripcionUbicacion() + " " + cpa.getNumero() + ubicacionesDep + "\n";
                }
                if (cpa.getNombre().contains("Bo")) {
                    ubicacionesBo = cpa.getNombre() + " " + cpa.getDescripcionUbicacion() + " " + cpa.getNumero() + ubicacionesBo + "\n";
                }
            }
            cp.setPropietarios(ubicacionesDep + " " + ubicacionesBo);
            catPredios.add(cp);
        }
        this.prediosGenerados = new ArrayList();
        this.prediosGenerados = catPredios;
    }
            ...
            switch (this.escritura.getTipoPh()) {
                case 1: // PHH
                    for (int i = 0; i < escritura.getCantAlicuotas(); i++) {
                        prediosGenerados.add(copyPredio(1, Short.valueOf("" + (i + 1)), Short.valueOf("0"), Short.valueOf("0"), numP));
                        numP = numP.add(new BigInteger("1"));
                    }
                    break;
